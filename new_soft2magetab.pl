...
my ( $soft, $target, $data, $skip_data, $gse_acc, $help );
...
if ( my $efo = $CONFIG->get_EFO_OWL_FILE ) {
...
my $converter = EBI::FGPT::Converter::GEO::SOFTtoMAGETAB->new(
	{
		soft_path     => $soft,
		acc           => $gse_acc,
		target_dir    => $target,
...
foreach my $sdrf_type (@sdrf_type) {
	...
	if ( -e $sdrf_path ) {
		...
		my @sdrf_args = (
'$ATLAS_PROD/sw/atlasinstall_prod/atlasprod/geo_import/sdrf_protocol_correct.pl',
		...
		@sdrf_args = (
'$ATLAS_PROD/sw/atlasinstall_prod/atlasprod/geo_import/sdrf_termsource_correct.pl',
			'-s', $sdrf_path
		);

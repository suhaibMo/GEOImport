...
my ( $listfile, $set_curator, $skip_download, $validate );
...
unless ($listfile)
{
    ...
    Usage: $0 -f <file listing GEO subs>
...
while ( my $line = <$list_fh> )
{
	...
	my $target_dir =
	  File::Spec->catdir( $CONFIG->get_GEO_STAGING_DIR(), "${accn}_output" );
	unless ( -e $target_dir )
	{
		mkpath($target_dir)
